...
function calcularPromedio(listaDeNumeros) {
    const sumatotal=listaDeNumeros.reduce((total,actual)=>{
        total+=actual;
        return total;
    },0);
...
function ordenarDeMayorAMenor(listaDeNumeros) {
    return listaNumerosEjemplo= listaDeNumeros.sort().reverse();
}
...
function filtrarSoloTruthy(listaDeValores) {
    return listaDeValores.filter(e=> e ? true : false);
}
...
function enumerarLista(listaDePalabras) {
    ...
    return listaDePalabras.join(",") + " y " + ultimoelemento + ".";   
}
